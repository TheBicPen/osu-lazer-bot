...
sleep $SLEEP_ON_LAUNCH
echo "Replay should now be loaded. Sleeping for $REPLAY_SLEEP seconds"
...
ffmpeg -video_size 1280x720 -framerate 60 -f x11grab -i :0.0 -f pulse -ac 2 -i default -t "$REPLAY_LENGTH" "$OUTPUT_FILE"
kill $OSU_PID
